...
#define UMEM_SIZE (32 * 1024 * 1024)   // 2 MB: large enough for ~1000 concurrent blocks
...
int run_threads(const char *filename) {
    ...
    pthread_t *threads = malloc(sizeof(pthread_t) * max_blocks);
    thread_arg_t *thread_args = malloc(sizeof(thread_arg_t) * max_blocks);
    unsigned long *results_array = malloc(sizeof(unsigned long) * max_blocks);
    
    if (!threads || !thread_args || !results_array) {
